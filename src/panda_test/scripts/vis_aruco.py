        ...
        for row in reader:
            ...
            if row[0]:  # If first column (pose) is not empty
                try:
                    ...
                    # Handle cases where row[0] is not a number (like '000021.jpg')
                    continue
...
def visualize_aruco_centers_on_images(image_folder, centers):
    ...
    for pose, coords in centers.items():
        # Construct the image filename based on the pose
        image_path = os.path.join(image_folder, f"{str(pose).zfill(6)}.jpg")
...
if __name__ == "__main__":
    csv_path = "/home/jc-merlab/Pictures/panda_data/ur10/aruco/aruco_centers.csv"
    image_folder = "/home/jc-merlab/Pictures/panda_data/ur10/aruco/"
