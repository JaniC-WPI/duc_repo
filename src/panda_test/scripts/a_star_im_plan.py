...
def is_point_too_close_to_obstacle(point, obstacle_center, safe_distance):
    """Check if a point is too close to the obstacle."""
    return np.linalg.norm(np.array(point) - np.array(obstacle_center)) < safe_distance
...
def is_line_too_close_to_obstacle(p1, p2, obstacle_center, safe_distance):
    ...
    for point in line_points(p1, p2):
        if is_point_too_close_to_obstacle(point, obstacle_center, safe_distance):
            return True
    return False
...
# def is_config_valid(configuration, obstacle_center, safe_distance):
#     """Check if any part of the robot configuration is too close to the obstacle."""
#     for i in range(len(configuration) - 1):
#         if is_line_too_close_to_obstacle(configuration[i], configuration[i+1], obstacle_center, safe_distance):
#             return True
...
def interpolate_configurations(config1, config2, steps=5):
    # Generates a few intermediate configurations between config1 and config2
    interpolated_configs = []
    for t in range(1, steps):
        intermediate_config = config1 + (config2 - config1) * t / steps
        interpolated_configs.append(intermediate_config)
...
def a_star_search(start_config, goal_config, configurations, obstacle_center, safe_distance):
    ...
    while open_set:
        ...
        if np.array_equal(current_config, goal_config):
            # Convert indices back to configurations, adding intermediate steps
            full_path = [start_config]
            for i in range(1, len(path_indices)):
                prev_config = configurations[path_indices[i - 1]]
                next_config = configurations[path_indices[i]]

                # Add intermediates only if the configurations are significantly different
                if config_distance(prev_config, next_config) > 100:
                    intermediates = interpolate_configurations(prev_config, next_config)
                    full_path.extend(intermediates)
                full_path.append(next_config)
        ...
        for next_index, next_config in enumerate(configurations):
            ...
            # Generate and check intermediate configurations
            intermediates = interpolate_configurations(current_config, next_config)
            if any(is_configuration_too_close_to_obstacle(intermediate, obstacle_center, safe_distance) for intermediate in intermediates):
                continue  # Skip if any intermediate configuration is invalid

            next_cost = current_cost + config_distance(current_config, next_config) - heuristic(current_config)
            new_path_indices = path_indices + [next_index]  # Add index of next configuration
