...
for path_template, label in zip(jt_file_paths_template, labels):
    for exp in exp_no:
        ...
        print("Interpolation points per experiment:", interpolation_points)

        for i, num_points in enumerate(interpolation_points):
            print(f"Processing Config {i} to Config {i+1} with {num_points} interpolation points.")

            # Interpolate joint angles between the configurations
            segment = interpolate_joints(joints[i], joints[i + 1], num_points)

            interpolated_keypoints = []
            actual_interpolated_keypoints = []

            # Generate interpolated keypoints for the segment
            ...
            # Generate ground-truth interpolated keypoints for the segment
            ...
            # Calculate deviations for the current interpolation segment
            deviations, deviations_per_keypoint = calculate_deviations(interpolated_keypoints, actual_interpolated_keypoints)

            # Sum total deviation for this segment
            total_deviation = sum(deviations)

            # Store total deviations and statistics for each segment
            deviation_stats.append({
                "Experiment": exp,
                "Roadmap": label,
                "Config Pair": f"{i}-{i+1}",
                "Total Interpolation Points": num_points,
                "Total Deviation": total_deviation,
                **{f"Total Dev kp{i}": sum(deviations_per_keypoint[i]) for i in range(1, 6)},
                **{f"Mean Dev kp{i}": np.mean(deviations_per_keypoint[i]) for i in range(1, 6)},
                **{f"Max Dev kp{i}": np.max(deviations_per_keypoint[i]) for i in range(1, 6)},
...
deviation_stats_df.to_csv("/media/jc-merlab/Crucial X9/paper_data/deviation_statistics_per_segment.xlsx", index=False)
print("Deviation statistics saved to 'deviation_statistics_per_segment.csv'")
