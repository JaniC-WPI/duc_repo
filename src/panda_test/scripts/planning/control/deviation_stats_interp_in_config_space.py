...
def visualize_interpolated_keypoints(original_keypoints, interpolated_keypoints, actual_interpolated_keypoints):
    ...
    for i, act_ip_config in enumerate(actual_interpolated_keypoints):
        ...
        ax.scatter(act_ip_config[:, 0], -act_ip_config[:, 1], color='green', label='Actual Interpolated Keypoints', s=5)
        # Label the keypoints in order
...
for path_template, label in zip(jt_file_paths_template, labels):
    for exp in exp_no:
        ...
        print("interpolation points per exp", interpolation_points)
        ...
        # print(exp)
        # print("Number of points", len(actual_interpolated_keypoints))  

        # Calculate deviations
        deviations, deviations_per_keypoint = calculate_deviations(interpolated_keypoints, actual_interpolated_keypoints)
        ...
        for kp_idx in deviations_per_keypoint:
            ...
            mean_deviation_kp = total_deviation_kp / 5000
        ...
        deviation_stats.append({
            ...
            # "Total Interpolated Points": len(interpolated_keypoints),
            # "Total Deviations": sum(deviations),
            **{f"Total Dev kp{i}": stats[i]["Total Deviation"] for i in range(1, 6)},
            # **{f"Max Deviation kp{i}": stats[i]["Max Deviation"] for i in range(1, 6)},
            **{f"Mean Dev kp{i}": stats[i]["Mean Deviation"] for i in range(1, 6)},
        })

# Save the results to a CSV file
deviation_stats_df = pd.DataFrame(deviation_stats)
print(deviation_stats_df)
deviation_stats_df.to_csv("/media/jc-merlab/Crucial X9/paper_data/deviation_statistics_no_obs_uniform_per_config_pair.xlsx", index=False)
print("Deviation statistics saved to 'deviation_statistics_no_obs_weighted.xlsx'")
