...
def interpolate_joints(joint_start, joint_end, num_interpolations=1):
    """Interpolates joint angles between two configurations."""
    return np.linspace(joint_start, joint_end, num_interpolations)

def interpolate_keypoints(kp_start, kp_end, num_interpolations):
    kp_start = np.array(kp_start)
    kp_end = np.array(kp_end)

    actual_interp_kp = np.linspace(kp_start, kp_end, num_interpolations, axis=0)
...
def visualize_interpolated_keypoints(original_keypoints, interpolated_keypoints, actual_interpolated_keypoints):
    ...
    for i, og_config in enumerate(original_keypoints):
        ...
        ax.scatter(og_config[:, 0], -og_config[:, 1], color='red', label='Original Keypoints', s=30)
...
panda = PandaReal2D()
# Test with sample joint angles (replace with actual joint angles)
joints = [-0.0027749391687434226, -0.980730831869426, 0.6590925264706217]
joint_angles = create_jt_matrix(joints)
# print(joint_angles)
...
keypoints = image_pixels(camera_K, camera_ext,three_d_proj)

# print(keypoints)

unique_kp = remove_duplicates(keypoints)
...
for idx, (x, y) in enumerate(keypoints):
    # Flip y-coordinate to align with image space
    # y = image_height - y
    x, y = int(x), int(y)
    cv2.circle(image, (x, y), 5, (0, 0, 255), -1)  # Red circles for keypoints
    # cv2.putText(image, str(idx), (x + 5, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
...
for path_template, label in zip(jt_file_paths_template, labels):
    for exp in exp_no:
        ...
        for i in range(len(joints) - 1):
            num_interp = int(num_interpolations/(len(joints) - 1))
            print("actual_number_of_interpolation between config pairs", num_interp)
            segment = interpolate_joints(joints[i], joints[i + 1], num_interp)
            for interp_joint in segment:            
                ...
                # print("initial_kp", initial_keypoints)

                unique_kp = remove_duplicates(initial_keypoints)

                # print("Unie_kp", unique_kp)

                updated_kp = update_keypoints(unique_kp)

                updated_kp = np.array(updated_kp)[1:, :]

                # print(np.array(updated_kp))

                interpolated_keypoints.append(updated_kp)

            actual_interp_kp = interpolate_keypoints(keypoints[i], keypoints[i+1], num_interp)
            actual_interpolated_keypoints.extend(actual_interp_kp)  
        print(exp)
        print("NUmber of points", len(interpolated_keypoints))    

        # Calculate deviations without flattening
        deviations = []
        deviations_sign = []

        for act_interp_kp, config_interp_kp in zip(actual_interpolated_keypoints, interpolated_keypoints):
            for actual_point, interpolated_point in zip(act_interp_kp, config_interp_kp):
                # distance = np.linalg.norm(np.array(actual_point) - np.array(interpolated_point))
                distance = np.linalg.norm(np.array(actual_point) - np.array(interpolated_point))
                manhattan_dist = np.sum(np.abs(actual_point - interpolated_point))
                dist_with_sign = np.array(interpolated_point) - np.array(actual_point)
                # deviations.append(distance)
                deviations.append(manhattan_dist)
                deviations_sign.append(dist_with_sign)

        print("Check Deviation", len(deviations))
        # Calculate deviations without flattening
        deviations_per_keypoint = {i: [] for i in range(1, 6)}

        for act_interp_kp, config_interp_kp in zip(actual_interpolated_keypoints, interpolated_keypoints):
            for i in range(1, 6):  # Keypoints from index 1 to 5
                actual_point = act_interp_kp[i]
                interpolated_point = config_interp_kp[i]
                manhattan_dist = np.sum(np.abs(actual_point - interpolated_point))
        ...
        total_deviation_sign = sum(deviations_sign)
        ...
        #     "Mean Deviation Per Exp": (total_deviation/total_points),
        #     "Max Deviation": max_deviation
        #     # "Mean deaviations signed": (total_deviation_sign/total_points)
        # })      


        # print("deviations per keypoints", deviations_per_keypoint)

        # Compute statistics
        stats = {}
        num_config_pairs = len(joints) - 1 
        total_points = int(num_interpolations/num_config_pairs)
        for kp_idx in deviations_per_keypoint:
            total_deviation_kp = sum(deviations_per_keypoint[kp_idx])
            max_deviation_kp = max(deviations_per_keypoint[kp_idx])
            # mean_deviation_kp = total_deviation_kp / len(deviations_per_keypoint[kp_idx])
            mean_deviation_kp = total_deviation_kp / total_points

            stats[kp_idx] = {
                "Total Deviation": total_deviation_kp,
                "Max Deviation": max_deviation_kp,
                "Mean Deviation": mean_deviation_kp,
            }

        # print("Interpolated_kp", interpolated_keypoints)
        # print("Actual Interpolated KP", np.array(actual_interpolated_keypoints).shape)
        # print("Config_space_interpolated_kp", np.array(interpolated_keypoints).shape)
        ...
        deviation_stats.append({
            ...
            "Roadmap": label,
            "Total Points": total_points,
            **{f"Total Deviation kp{i}": stats[i]["Total Deviation"] for i in range(1, 6)},
            **{f"Max Deviation kp{i}": stats[i]["Max Deviation"] for i in range(1, 6)},
            **{f"Mean Deviation kp{i}": stats[i]["Mean Deviation"] for i in range(1, 6)},
        })
...
# deviation_stats_df.to_csv("/media/jc-merlab/Crucial X9/paper_data/deviation_statistics_no_obs_uniform.xlsx", index=False)
# print("Deviation statistics saved to 'deviation_statistics_no_obs.xlsx'")
