...
class Node:
    ...
    def is_similar(self, other_node, threshold=7):
...
def a_star_search(start_config, goal_config, model_path, velocities, output_csv_path):
    ...
    with open(output_csv_path, mode='w', newline='') as file:
        ...
        while len(open_list) > 0 and iteration < max_iterations:
            ...
            for velocity in velocities:
                ...
                # Log iteration, velocity, and new configuration to CSV
                writer.writerow([iteration, velocity, current_node.configuration.tolist(), new_config.tolist()])

                if any(new_node.is_similar(closed_node) for closed_node in closed_list):
                    continue

                new_node.g = current_node.g + config_distance(current_node.configuration, new_config)
                new_node.h = heuristic(new_node.configuration, goal_config)
                new_node.f = new_node.g + new_node.h

                if not any(open_node for open_node in open_list if new_node == open_node and new_node.g > open_node.g):
...
if __name__ == '__main__':
    ...
    csv_path = '/home/jc-merlab/Pictures/panda_data/panda_sim_vel/csv/path_predictions.csv'

    directory = '/home/jc-merlab/Pictures/panda_data/panda_sim_vel/unique_vel_planning/2/'
    velocities = load_velocities_from_directory(directory)
    model_path = '/home/jc-merlab/Pictures/Data/trained_models/reg_nkp_panda_b64_e100_v1_l5.pth'

    # Detect the obstacle (red ball)
    image_path = '/home/jc-merlab/Pictures/panda_data/panda_sim_vel/regression_dataset_panda/000002.jpg'  # Replace with the path to your image file
    ...
    path = a_star_search(start_config, goal_config, model_path, velocities, csv_path)
